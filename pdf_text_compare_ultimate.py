...
class PDFViewer(QScrollArea):
    ...
    def is_meaningless_word(self, word):
        ...
        """
        # URL 제거
        if 'http' in word.lower() or 'www.' in word.lower() or '.com' in word.lower() or '.go.kr' in word.lower():
            return True
        ...
        # 단일 문자이고 특수문자인 경우
        if len(word.strip()) == 1 and not word.strip().isalnum() and not self.is_korean(word.strip()):
            return True
    ...
    def extract_text_with_word_info(self, page_num, rect):
        """선택 영역에서 텍스트와 단어 정보 추출 (강력한 정규화 포함)"""
        if not self.pdf_doc:
            return
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PDF 텍스트 비교 도구 (최종 개선 버전)")
    ...
    def compare_texts(self):
        ...
        try:
            ...
            result_html += f"<p><b>정규화 적용:</b> 불릿 포인트, URL, 구두점 제거, 대소문자 통일</p>"
