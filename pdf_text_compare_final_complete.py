...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QLabel, QFileDialog, QScrollArea, QMessageBox, QTextEdit, QCheckBox
)
...
try:
    ...
    print("⚠️ KoNLPy가 설치되지 않았습니다. 형태소 분석 기능이 비활성화됩니다.")
            ...
            if len(left_word) < len(right_word):
                for k in range(1, min(5, len(words_left) - i)):  # 최대 5개 단어까지 합침
                    ...
                    if combined == right_word:
                        ...
                        # 왼쪽 i ~ i+k-1을 '삭제'로 표시 (실제로는 합쳐진 것)
            ...
            # 오른쪽이 더 짧은 경우: 오른쪽 단어들을 합쳐서 왼쪽과 비교
                ...
                for k in range(1, min(5, len(words_right) - j)):  # 최대 5개 단어까지 합침
                    ...
                    if combined == left_word:
                        ...
                        # 오른쪽 j ~ j+k-1을 '추가'로 표시 (실제로는 합쳐진 것)
...
class PDFViewer(QScrollArea):
            ...
            if unit in text:
                try:
                    ...
                    if match:
                        ...
                        print(f"  숫자 변환: {match.group(0)} → {result}")
                except Exception as e:
                    # 변환 실패 시 원본 유지
                    pass
    ...
    def normalize_word(self, word, use_morphs=False):
        """
        강화된 단어 정규화
        - 의미 없는 단어 제거
        - 구두점 제거
        ...
        # 6. 형태소 분석 (선택적)
        if use_morphs and self.okt and word:
            try:
                # 명사만 추출
                morphs = self.okt.nouns(word)
                if morphs:
                    word = ' '.join(morphs)
            except Exception as e:
                # 형태소 분석 실패 시 원본 사용
    ...
    def extract_text_with_word_info(self, page_num, rect, use_morphs=False):
        """선택 영역에서 텍스트와 단어 정보 추출 (강화된 정규화 포함)"""
        if not self.pdf_doc:
            return
        ...
        try:
            ...
            for word_tuple in words:
                ...
                if selection_rect.intersects(word_bbox):
                    # 정규화
                    normalized = self.normalize_word(word_text, use_morphs)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PDF 텍스트 비교 도구 (최종 완성판)")
        ...
        features = ["재동기화 로직 (핵심!)", "한글 숫자 단위 변환 (NEW!)", "불릿 포인트 제거 (30종)", "URL 제거", "구두점 제거", "공백 정규화", "대소문자 통일"]
        if KONLPY_AVAILABLE:
            features.append("형태소 분석 (선택적)")
        
        self.result_text.setHtml(f"""
        <p><b>📌 최종 완성판 (최고 정확도)</b></p>
        <ul>
        {''.join([f'<li>{f}</li>' for f in features])}
        </ul>
        <p><b>예시:</b> "1,000만" ↔ "10,000,000" → 같은 숫자로 인식!</p>
    ...
    def create_controls(self):
        ...
        # 형태소 분석 옵션 (KoNLPy 사용 가능한 경우만)
        if KONLPY_AVAILABLE:
            self.chk_use_morphs = QCheckBox("형태소 분석 사용")
            self.chk_use_morphs.setToolTip("한글 명사만 추출하여 비교 (더 정확하지만 느림)")
        else:
            self.chk_use_morphs = None
        ...
        if self.chk_use_morphs:
            control_layout.addWidget(self.chk_use_morphs)
    ...
    def compare_texts(self):
        ...
        try:
            print("\n" + "="*60)
            print("텍스트 비교 시작 (재동기화 버전)")
            print("="*60)
            
            # 형태소 분석 사용 여부
            use_morphs = self.chk_use_morphs.isChecked() if self.chk_use_morphs else False
            
            if use_morphs:
                print("✓ 형태소 분석 활성화")
                # 형태소 분석으로 재추출
                if self.viewer_left.selected_page >= 0:
                    rect = self.viewer_left.page_labels[self.viewer_left.selected_page].get_selection_rect()
                    if rect:
                        self.viewer_left.extract_text_with_word_info(self.viewer_left.selected_page, rect, use_morphs=True)
                
                if self.viewer_right.selected_page >= 0:
                    rect = self.viewer_right.page_labels[self.viewer_right.selected_page].get_selection_rect()
                    if rect:
                        self.viewer_right.extract_text_with_word_info(self.viewer_right.selected_page, rect, use_morphs=True)
            ...
            # 재동기화 로직으로 비교
            print(f"\n[재동기화 비교 시작]")
            differences_raw = compare_with_resync(words_left, words_right, lookahead=5)
            
            print(f"\n[비교 결과]")
            print(f"차이점 수: {len(differences_raw)}")
            ...
            for diff_type, left_idx, right_idx in differences_raw:
                if diff_type == 'delete':
                    if left_idx is not None and left_idx < len(word_info_left):
                        differences.append(f"❌ 삭제: '{word_info_left[left_idx]['text']}'")
                        left_diffs.append({
                            'word': word_info_left[left_idx]['text'],
                            'type': 'delete',
                            'bbox': word_info_left[left_idx]['bbox'],
                            'page': word_info_left[left_idx]['page']
                        })
                elif diff_type == 'insert':
                    if right_idx is not None and right_idx < len(word_info_right):
                        differences.append(f"✅ 추가: '{word_info_right[right_idx]['text']}'")
                        right_diffs.append({
                            'word': word_info_right[right_idx]['text'],
                            'type': 'add',
                            'bbox': word_info_right[right_idx]['bbox'],
                            'page': word_info_right[right_idx]['page']
                        })
                elif diff_type == 'replace':
                    if left_idx is not None and left_idx < len(word_info_left):
                        differences.append(f"🔄 변경: '{word_info_left[left_idx]['text']}'")
                        left_diffs.append({
                            'word': word_info_left[left_idx]['text'],
                            'type': 'change',
                            'bbox': word_info_left[left_idx]['bbox'],
            ...
            morph_status = "형태소 분석 + 재동기화" if use_morphs else "재동기화"
            result_html = f"<h3>✅ 비교 완료! (유사도: {ratio * 100:.1f}%)</h3>"
            result_html += f"<p><b>비교 모드:</b> {morph_status}</p>"
            result_html += f"<p><b>적용:</b> 재동기화 로직, 불릿 포인트 제거, URL 제거, 구두점 제거, 공백 정규화, 대소문자 통일</p>"
...
if __name__ == '__main__':
    try:
        ...
        print("=" * 60)
        print("PDF 텍스트 비교 도구 (재동기화 버전)")
        print("=" * 60)
        print("핵심 기능:")
        print("- 재동기화 로직 (1단어 불일치 시 자동 재정렬)")
        print("- 불릿 포인트 제거 (30종 이상)")
        print("- URL 제거")
        print("- 구두점 제거")
        print("- 공백 정규화")
        print("- 대소문자 통일")
