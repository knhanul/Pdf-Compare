...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PDF 텍스트 비교 도구 (유사도 향상 버전)")
        ...
        features = ["불릿 포인트 제거 (30종 이상)", "URL 제거", "구두점 제거", "공백 정규화", "대소문자 통일"]
        if KONLPY_AVAILABLE:
    ...
    def compare_texts(self):
        ...
        try:
            ...
            morph_status = "형태소 분석 적용" if use_morphs else "일반 정규화"
            result_html = f"<h3>✅ 비교 완료! (유사도: {ratio * 100:.1f}%)</h3>"
            result_html += f"<p><b>정규화 모드:</b> {morph_status}</p>"
            result_html += f"<p><b>적용:</b> 불릿 포인트, URL, 구두점 제거, 공백 정규화, 대소문자 통일</p>"
