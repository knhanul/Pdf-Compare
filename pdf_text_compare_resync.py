...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PDF 텍스트 비교 도구 (재동기화 버전)")
        ...
        features = ["재동기화 로직 (핵심!)", "불릿 포인트 제거 (30종)", "URL 제거", "구두점 제거", "공백 정규화", "대소문자 통일"]
        if KONLPY_AVAILABLE:
            features.append("형태소 분석 (선택적)")
        
        self.result_text.setHtml(f"""
        <p><b>📌 재동기화 버전 (최고 정확도)</b></p>
        <ul>
